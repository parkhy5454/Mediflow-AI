...
// PDF/Excel 내보내기는 React 컴포넌트가 아니라 순수 JS 함수라 useTranslation 훅을
// 쓸 수 없다. 대신 i18next 싱글턴의 t()를 직접 호출해서, 화면에서 선택된 언어
// 그대로 인쇄/내보내기 결과물에도 반영되게 한다.
const t = (...args) => i18n.t(...args);

// 요일 표기(예: 화/Tue)는 브라우저 로케일 문자열이 필요해서 t()로는 못 바꾼다.
// 현재 앱 언어 → Intl 로케일로 매핑.
const LOCALE_BY_LANG = { ko: 'ko-KR', en: 'en-US', zh: 'zh-CN' };
const currentLocale = () => LOCALE_BY_LANG[i18n.language?.slice(0, 2)] || 'ko-KR';

const validateRosterData = (monthRoster) => {
  if (!monthRoster || Object.keys(monthRoster).length === 0) {
    throw new Error(t('내보낼 근무표 데이터가 없습니다.'));
  }
  ...
  if (!hasValidData) {
    throw new Error(t('근무표 데이터가 비어있거나 올바르지 않습니다.'));
  }
...
export const exportToExcel = async (monthRoster, selectedMonth, selectedYear, rosterConfig) => {
  try {
    ...
    return { success: true, message: t('{{year}}년 {{month}} 엑셀 파일이 성공적으로 내보내졌습니다.', { year: selectedYear, month: monthName }) };

  } catch (error) {
    console.error('Excel export error:', error);
    throw new Error(t('엑셀 파일 내보내기에 실패했습니다: {{message}}', { message: error.message }));
  }
};
...
export const exportToPDF = (monthRoster, selectedMonth, selectedYear, rosterConfig, nurses) => {
  try {
    ...
    if (!printWindow) {
      throw new Error(t('인쇄 창을 열 수 없습니다. 브라우저의 팝업 차단 설정을 확인해주세요.'));
    }
    ...
    let htmlContent = `
      ...
      <html>
      <head>
        <title>${t('병원 간호사 근무표 - {{year}}년 {{month}}', { year: selectedYear, month: monthName })}</title>
        <meta charset="UTF-8">
        <style>
      ...
      <body>
        <div class="no-print" style="background:#eff6ff;border:1px solid #bfdbfe;color:#1e40af;padding:10px 14px;border-radius:8px;margin-bottom:16px;font-size:13px;">
          💡 ${t('인쇄 대화상자에서 "추가 설정" → "머리글 및 바닥글" 체크를 해제하면, 페이지마다 뜨는 날짜/URL/페이지 번호 없이 표가 더 깔끔하게 이어져 보입니다.')}
        </div>
        <div class="header no-break">
          <h1>🏥 ${t('병원 간호사 근무표 시스템')}</h1>
          <h2>${t('{{year}}년 {{month}}', { year: selectedYear, month: monthName })}</h2>
          <div class="meta">${t('생성일: {{date}} {{time}}', { date: new Date().toLocaleDateString(currentLocale()), time: new Date().toLocaleTimeString(currentLocale()) })}</div>
        </div>
        ...
        <div class="section no-break">
          ...
          <div class="config-grid">
            ${shiftTypes.map(s => {
              const cfg = rosterConfig.shifts[s];
              return `<div class="config-item"><strong>${t(shiftFullLabel(s))}:</strong> ${t('{{size}}명 / 연속 {{shiftDays}}일 근무 / 휴무 {{offDutyAfter}}일', { size: cfg.size, shiftDays: cfg.shiftDays, offDutyAfter: cfg.offDutyAfter })}</div>`;
            }).join('')}
          </div>
        </div>
        ...
        <div class="section">
          ...
          <table class="roster-table">
            <thead>
              <tr>
                <th>${t('export.dayNumberColumn', { defaultValue: '일' })}</th>
                <th>${t('날짜')}</th>
                ${shiftTypes.map(s => `<th>${t(shiftFullLabel(s))} (${rosterConfig.shifts[s].size})</th>`).join('')}
                <th>${t('비번 (OFF)')}</th>
              </tr>
            </thead>
            <tbody>
      ...
      for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (dayData?.offDuty) {
          const offDutyBadges = dayData.offDuty.map(nurse => {
            ...
            const status = nurse.daysRemaining > 0 ? ' ' + t('({{days}}일)', { days: nurse.daysRemaining }) :
                         nurse.status === 'Available' ? ' ' + t('(근무 가능)') : '';
          ...
          <table class="workload-table">
            <thead>
              <tr>
                <th>${t('간호사 이름')}</th>
                <th>${t('자격')}</th>
                ${shiftTypes.map(s => `<th>${t(shiftLabel(s))}</th>`).join('')}
                <th>${t('총 근무일')}</th>
                <th>${t('휴무일')}</th>
              </tr>
            </thead>
        ...
        <div class="section">
          ...
          <div class="summary-stats">
            ...
            <div class="stat-card">
              ...
              <div class="stat-label">${t('총 {{shift}} 근무', { shift: t(shiftLabel(s)) })}</div>
            </div>`).join('')}
            <div class="stat-card">
              <div class="stat-number">${totalOffDutyDays}</div>
              <div class="stat-label">${t('총 휴무일')}</div>
            </div>
            <div class="stat-card">
              <div class="stat-number">${workloadSummary.length}</div>
              <div class="stat-label">${t('근무 중인 간호사')}</div>
            </div>
            <div class="stat-card">
              <div class="stat-number">${averageWorkDays}</div>
              <div class="stat-label">${t('간호사당 평균 근무일')}</div>
            </div>
            <div class="stat-card">
              <div class="stat-number">${daysInMonth}</div>
              <div class="stat-label">${t('이번 달 일수')}</div>
            </div>
          </div>
        </div>

        <div class="footer">
          <p>${t('이 근무표는 병원 간호사 근무 관리 시스템에 의해 자동으로 생성되었습니다.')}</p>
          <p>${t('© {{year}} 병원 관리 시스템 | 생성 시각: {{time}}', { year: new Date().getFullYear(), time: new Date().toLocaleString(currentLocale()) })}</p>
        </div>
      </body>
      </html>
    `;
    ...
    return { success: true, message: t('{{year}}년 {{month}} PDF 내보내기가 시작되었습니다.', { year: selectedYear, month: monthName }) };

  } catch (error) {
    console.error('PDF export error:', error);
    throw new Error(t('PDF 내보내기에 실패했습니다: {{message}}', { message: error.message }));
  }
};
